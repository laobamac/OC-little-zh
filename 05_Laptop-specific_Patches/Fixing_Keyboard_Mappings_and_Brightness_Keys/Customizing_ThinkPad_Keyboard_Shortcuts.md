...
Enabling ACPI debugging is easy: grab `SSDT-RMDT.aml` (RehabMan Debugging Table) and `ACPIDebug.kext` from the [**ACPI Debuggig section**](/00_ACPI/ACPI_Debugging) and add them to your EFI and config.plist. The (relatively) difficult part is writing an SSDT for debugging.

The sample [`SSDT-BKeyQxx-Debug.dsl`](/00_ACPI/ACPI_Debugging/SSDT-BKeyQxx-Debug.dsl) in OC-little also provides an example for using the RMDT function that prints two arguments:
